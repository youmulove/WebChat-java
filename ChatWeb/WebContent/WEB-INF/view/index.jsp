...
<div class="am-cf admin-main">
    ...
    <div class="admin-content">
        <div class="" style="width: 80%;float:left;">
            <!-- 聊天区 -->
            <div class="am-scrollable-vertical" id="chat-view" style="height: 455px;">
                <ul class="am-comments-list am-comments-list-flip" id="chat">
                </ul>
            </div>
            <!-- 输入区 -->
            <div class="am-form-group am-form" style= "height: 40px;">
...
<script>
    ...
    function video(item){
      ...
      * 视频
      */
     function videoChat(item){
	<%-- var url = "<%=path%>/video/msg?uid=${userid}"; --%>
     layer.open({
 		  type: 2,
 		  title: false,
 		  closeBtn:0,
 		  ...
 		  maxmin: true,
 		  scrollbar: false,
 		  content:'<%=path%>/video/msg?uid=${userid}',
...
//       音频
	function audioChat(item){
<%--         	var url = "<%=path%>/audio/msg?uid=${userid}"; --%>
     layer.open({
 		  type: 2,
 		  title: false,
 		  closeBtn:0,
 		  ...
 		  maxmin: true,
 		  scrollbar: false,
 		  content:'<%=path%>/audio/msg?uid=${userid}',
 		  end : function() {
 			 addChat(item);
 	         var message = $("#message").val();
 	         var to = $("#sendto").text() == "全体成员"? "": $("#sendto").text();
    ...
    function analysisMessage(message){
        ...
        if(message.type == "video"){       //视频消息
            layer.open({
            	area: ['600px', '200px'],
    		    content:message.message.from+"邀请你视频通话",
    		    btn: ['确定','取消'],
    		    yes: function(index, layero){
    		        ...
    		        layer.open({
    						  ...
    						  maxmin: true,
    						  scrollbar: false,
    						  content:'<%=path%>/video/msg?uid=${userid}&oid=' +message.message.from,
    						  end : function() {
    							window.location.reload();
    							}
    						});
        ...
        if(message.type == "audio"){       
            layer.open({
            	area: ['600px', '200px'],
    		    content:message.message.from+"邀请你语音通话",
    		    btn: ['确定','取消'],
    		    yes: function(index, layero){
    		        //事件
    		        layer.close(index);
    		        layer.open({
    						  ...
    						  maxmin: true,
    						  scrollbar: false,
    						  content:'<%=path%>/audio/msg?uid=${userid}&oid=' +message.message.from,
    						  end : function() {
    							window.location.reload();
    							}
    						});
    ...
    function showOnline(list){
        ...
        $.each(list, function(index, item){     //添加私聊按钮
            var li = "<li>"+item+"</li>";
            if('${userid}' != item){    //排除自己
                li = "<li>"+item+" <button type=\"button\" class=\"am-btn am-btn-xs am-btn-primary am-round\" onclick=\"addChat('"+item+"');\"><span class=\"am-icon-phone\"><span> 私聊</button></li>" + " <button type=\"button\" class=\"am-btn am-btn-xs am-btn-primary am-round\" onclick=\"videoChat('"+item+"');\"><span class=\"am-icon-phone\"><span> 视频</button></li>" + "<button type=\"button\" class=\"am-btn am-btn-xs am-btn-primary am-round\" onclick=\"audioChat('"+item+"');\"><span class=\"am-icon-phone\"><span> 语音</button></li>" ;
